...
graph TB
    ...
    subgraph PVE["Proxmox Host (192.168.1.253)"]
        subgraph VMBR0["vmbr0 -- Home LAN (192.168.0.0/23)"]
            ...
            NGINX["nginx-proxy<br/>192.168.1.105<br/>dual-homed"]
            PWEB["personal-web<br/>192.168.1.106"]
            RWEB["rakuen-web<br/>192.168.1.107"]
        end
...
- **nginx-proxy** (CT 105) -- dual-homed nginx reverse proxy with certbot TLS. Routes ackmud.com, aha.ackmud.com, bailes.us, rakuensoftware.com, and the rakuensoft.com redirect to their respective backends. LAN backends are addressed by DNS name, so renumbering one is a DNS change rather than a proxy edit.
- **dns** (CT 101) -- serves the `bailes.us` zone. Every container is created pointing at it, which is what lets services address each other by name. nginx-proxy depends on it to resolve its backends.
- **deploy** (CT 109) -- quad-homed deployment container. GitHub Actions SSHs in on :2222 to build and deploy artifacts across all networks. Key-only auth, GitHub IP allowlist.
- **personal-web** (CT 106) -- static file server (node serve on :3000) for bailes.us.

- **rakuen-web** (CT 107) -- static file server (node serve on :3000) for rakuensoftware.com. Builds the Vite/React site in-container.
...
| dns | 192.168.1.101 | 101 | LXC | Authoritative DNS for the `bailes.us` zone |
| personal-web | 192.168.1.106 | 106 | LXC | Static file server for bailes.us (:3000) |
| media-stack | 192.168.1.119 | 119 | LXC | Media automation stack (Prowlarr, Sonarr, Radarr, Lidarr, Readarr) |
| rakuen-web | 192.168.1.107 | 107 | LXC | Static file server for rakuensoftware.com (:3000) |
| nginx-proxy | 192.168.1.105 | 105 | LXC (dual-homed) | nginx reverse proxy + certbot TLS for all web sites |
| deploy | 192.168.1.109 | 109 | LXC (quad-homed) | GitHub Actions deployment (SSH :2222, builds + deploys) |
...
1. **Homelab** (`homelab/bootstrap/`) -- apt-cache (00), vpn-gateway (01), bittorrent (02), obs (03), nginx-proxy (06), personal-web (07), deploy (09), rakuen-web (13). obs must be deployed before WOL Promtail steps. deploy depends on all networks existing. No dependencies on ACK.
   ...
   nginx-proxy (06) is the exception to running these in order: its vhosts name their backends, so it needs the dns container (CT 101) serving the `bailes.us` zone with records for personal-web and rakuen-web already in place. It checks this and refuses to configure rather than writing a config that will not parse. The dns container is not part of this bootstrap set.
