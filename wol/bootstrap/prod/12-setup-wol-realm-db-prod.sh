#!/usr/bin/env bash
# 12-setup-wol-realm-db-prod.sh -- Set up PostgreSQL for wol-realm (prod)
#
# Runs on: wol-realm-db-prod (10.0.0.214) -- Debian 13 LXC
# Run order: After CA is running (for root_ca.crt distribution)
#
# Sets up a dedicated PostgreSQL instance for the prod wol-realm service:
...
#   - pg_hba with client cert auth from wol-realm-prod (10.0.0.210)
#   - Firewall allowing only wol-realm-prod and SSH
...
configure_pg_ssl() {
    ...
    openssl req -new -x509 \
        ...
        -days 365 \
        -nodes \
        -subj "/CN=wol-realm-db-prod/O=WOL Infrastructure" \
        -addext "subjectAltName=DNS:wol-realm-db-prod,IP:$DB_HOST"
...
# wol-realm-prod ($API_HOST) -- client cert auth
...
================================================================
wol-realm-db-prod setup complete ($DB_HOST).
