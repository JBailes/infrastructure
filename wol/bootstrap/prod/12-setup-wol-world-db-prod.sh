#!/usr/bin/env bash
# 17-setup-wol-world-db-prod.sh -- Set up PostgreSQL for wol-world (prod)
#
# Runs on: wol-world-db-prod (10.0.0.213) -- Debian 13 LXC
# Run order: After CA is running (for root_ca.crt distribution)
#
# Sets up a dedicated PostgreSQL instance for the prod wol-world service:
...
#   - pg_hba with client cert auth from wol-world-prod (10.0.0.211)
#   - Firewall allowing only wol-world-prod and SSH
...
configure_pg_ssl() {
    ...
    openssl req -new -x509 \
        ...
        -days 365 \
        -nodes \
        -subj "/CN=wol-world-db-prod/O=WOL Infrastructure" \
        -addext "subjectAltName=DNS:wol-world-db-prod,IP:$DB_HOST"
...
# wol-world-prod ($API_HOST) -- client cert auth
...
================================================================
wol-world-db-prod setup complete ($DB_HOST).
