#!/usr/bin/env bash
# 12-setup-wol-realm-db-test.sh -- Set up PostgreSQL for wol-realm (test)
#
# Runs on: wol-realm-db-test (10.0.1.219) -- Debian 13 LXC
# Run order: After CA is running (for root_ca.crt distribution)
#
# Sets up a dedicated PostgreSQL instance for the test wol-realm service:
#   - PostgreSQL 17 via pgdg
#   - Self-signed SSL cert (replaced by CA cert later)
#   - wol_realm database
#   - wol_realm user (runtime, cert auth)
#   - wol_realm_migrate user (DDL, cert auth)
#   - pg_hba with client cert auth from wol-realm-test (10.0.1.215)
#   - Firewall allowing only wol-realm-test and SSH
...
DB_NAME="wol_realm"
DB_USER="wol_realm"
MIGRATE_USER="wol_realm_migrate"
API_HOST="10.0.1.215"
...
configure_pg_ssl() {
    ...
    openssl req -new -x509 \
        ...
        -days 365 \
        -nodes \
        -subj "/CN=wol-realm-db-test/O=WOL Infrastructure" \
        -addext "subjectAltName=DNS:wol-realm-db-test,IP:$DB_HOST"
...
# wol-realm-test ($API_HOST) -- client cert auth
...
configure_firewall() {
    ...
    iptables -A INPUT -s 10.0.1.100 -p tcp --dport 9187 -j ACCEPT
    fw_enable
}
...
================================================================
wol-realm-db-test setup complete ($DB_HOST).
