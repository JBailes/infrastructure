#!/usr/bin/env bash
# 17-setup-wol-world-db-test.sh -- Set up PostgreSQL for wol-world (test)
#
# Runs on: wol-world-db-test (10.0.1.218) -- Debian 13 LXC
# Run order: After CA is running (for root_ca.crt distribution)
#
# Sets up a dedicated PostgreSQL instance for the test wol-world service:
...
#   - pg_hba with client cert auth from wol-world-test (10.0.1.216)
#   - Firewall allowing only wol-world-test and SSH
...
configure_pg_ssl() {
    ...
    openssl req -new -x509 \
        ...
        -days 365 \
        -nodes \
        -subj "/CN=wol-world-db-test/O=WOL Infrastructure" \
        -addext "subjectAltName=DNS:wol-world-db-test,IP:$DB_HOST"
...
# wol-world-test ($API_HOST) -- client cert auth
...
================================================================
wol-world-db-test setup complete ($DB_HOST).
