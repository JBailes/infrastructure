#!/usr/bin/env bash
# 15-setup-wol-world-test.sh -- Prepare wol-world-test host environment
#
# Runs on: wol-world-test (10.0.1.216) -- Debian 13 LXC (privileged)
# Run order: Step 12 (after SPIRE Agent and wol-world-db-test are running)
...
# (removed, cfssl CA address in lib/common.sh)
...
[Unit]
Description=WOL World API (test)
...
main() {
    ...
    write_systemd_unit
    configure_firewall
    info "wol-world-test host environment is ready."
}
